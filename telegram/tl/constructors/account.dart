...
class Authorizations {
  ...
  List<int> getBytes() {
    return [
      readBufferFromBigInt(307276766, 4),
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.authorizations.length, 4, little: true, signed: true),
      this.authorizations.map((x) => x.getBytes()),
    ].expand((element) => element).toList();
  }
...
class Password {
  ...
  Password(
      {this.hasRecovery, this.hasSecureValues, this.hasPassword, this.currentAlgo, this.srp_B, this.srpId, this.hint, this.emailUnconfirmedPattern, this.newAlgo, this.newSecureAlgo, this.secureRandom});
  ...
  static Password fromReader(BinaryReader reader) {
    ...
    var currentAlgo;
    if ((flags & 4) == 1) {
      var currentAlgo = reader.tgReadObject();
    } else {
      currentAlgo = null;
    }
    var srp_B;
    if ((flags & 4) == 1) {
      var srp_B = reader.tgReadBytes();
    } else {
      srp_B = null;
    }
    var srpId;
    ...
    var hint;
    if ((flags & 8) == 1) {
      var hint = reader.tgReadString();
    } else {
      hint = null;
    }
    var emailUnconfirmedPattern;
    if ((flags & 16) == 1) {
      var emailUnconfirmedPattern = reader.tgReadString();
    } else {
    ...
    return new Password(hasRecovery: hasRecovery,
  ...
  List<int> getBytes() {
    return [
      readBufferFromBigInt(2904965624, 4),
      [0, 0, 0, 0],
      (this.currentAlgo == null || this.currentAlgo == false) ? [] : [this.currentAlgo.getBytes()],
      (this.srp_B == null || this.srp_B == false) ? [] : [serializeBytes(this.srp_B)],
      (this.srpId == null || this.srpId == false) ? [] : [readBufferFromBigInt(this.srpId, 8, little: true, signed: true)],
      (this.hint == null || this.hint == false) ? [] : [serializeBytes(this.hint)],
      (this.emailUnconfirmedPattern == null || this.emailUnconfirmedPattern == false) ? [] : [serializeBytes(this.emailUnconfirmedPattern)],
...
class PasswordSettings {
  ...
  static PasswordSettings fromReader(BinaryReader reader) {
    ...
    var secureSettings;
    if ((flags & 2) == 1) {
      var secureSettings = reader.tgReadObject();
    } else {
      secureSettings = null;
    }
    return new PasswordSettings(email: email, secureSettings: secureSettings);
  }
  ...
  List<int> getBytes() {
    return [
      ...
      [0, 0, 0, 0],
      (this.email == null || this.email == false) ? [] : [serializeBytes(this.email)],
      (this.secureSettings == null || this.secureSettings == false) ? [] : [this.secureSettings.getBytes()],
    ].expand((element) => element).toList();
  }
...
class PasswordInputSettings {
  ...
  static PasswordInputSettings fromReader(BinaryReader reader) {
    ...
    var newPasswordHash;
    if ((flags & 1) == 1) {
      var newPasswordHash = reader.tgReadBytes();
    } else {
    ...
    var newSecureSettings;
    if ((flags & 4) == 1) {
      var newSecureSettings = reader.tgReadObject();
    } else {
      newSecureSettings = null;
    }
    return new PasswordInputSettings(newAlgo: newAlgo,
  ...
  List<int> getBytes() {
    return [
      ...
      [0, 0, 0, 0],
      (this.newAlgo == null || this.newAlgo == false) ? [] : [this.newAlgo.getBytes()],
      (this.newPasswordHash == null || this.newPasswordHash == false) ? [] : [serializeBytes(this.newPasswordHash)],
      (this.hint == null || this.hint == false) ? [] : [serializeBytes(this.hint)],
      (this.email == null || this.email == false) ? [] : [serializeBytes(this.email)],
      (this.newSecureSettings == null || this.newSecureSettings == false) ? [] : [this.newSecureSettings.getBytes()],
    ].expand((element) => element).toList();
  }
...
class TmpPassword {
  ...
  static TmpPassword fromReader(BinaryReader reader) {
    ...
    return new TmpPassword(tmpPassword: tmpPassword, validUntil: validUntil);
  }
...
class AuthorizationForm {
  ...
  static AuthorizationForm fromReader(BinaryReader reader) {
    ...
    if ((flags & 1) == 1) {
      var privacyPolicyUrl = reader.tgReadString();
    } else {
      privacyPolicyUrl = null;
    }
    return new AuthorizationForm(requiredTypes: requiredTypes,
        values: values,
        errors: errors,
        users: users,
  ...
  List<int> getBytes() {
    return [
      ...
      [0, 0, 0, 0],
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.requiredTypes.length, 4, little: true, signed: true),
      this.requiredTypes.map((x) => x.getBytes()),
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.values.length, 4, little: true, signed: true),
      this.values.map((x) => x.getBytes()),
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.errors.length, 4, little: true, signed: true),
      this.errors.map((x) => x.getBytes()),
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.users.length, 4, little: true, signed: true),
      this.users.map((x) => x.getBytes()),
      (this.privacyPolicyUrl == null || this.privacyPolicyUrl == false) ? [] : [serializeBytes(this.privacyPolicyUrl)],
    ].expand((element) => element).toList();
  }
...
class SentEmailCode {
  ...
  static SentEmailCode fromReader(BinaryReader reader) {
    ...
    return new SentEmailCode(emailPattern: emailPattern, length: length);
  }
...
class Takeout {
  ...
  static Takeout fromReader(BinaryReader reader) {
    ...
    return new Takeout(id: id);
  }

  List<int> getBytes() {
    return [readBufferFromBigInt(1304052993, 4), readBufferFromBigInt(this.id, 8, little: true, signed: true),]
...
class WallPapersNotModified {
  ...
  List<int> getBytes() {
    return [readBufferFromBigInt(471437699, 4),].expand((element) => element).toList();
  }
...
class WallPapers {
  ...
  List<int> getBytes() {
    return [
      readBufferFromBigInt(1881892265, 4),
      readBufferFromBigInt(this.hash, 4, little: true, signed: true),
      readBufferFromBigInt(0x15c4b51c, 4),
      readBufferFromBigInt(this.wallpapers.length, 4, little: true, signed: true),
      this.wallpapers.map((x) => x.getBytes()),
    ].expand((element) => element).toList();
  }
...
class AutoDownloadSettings {
  ...
  static AutoDownloadSettings fromReader(BinaryReader reader) {
    ...
    return new AutoDownloadSettings(low: low, medium: medium, high: high);
  }

  List<int> getBytes() {
    return [readBufferFromBigInt(1674235686, 4), this.low.getBytes(), this.medium.getBytes(), this.high.getBytes(),].expand((
...
class ThemesNotModified {
  ...
  List<int> getBytes() {
    return [readBufferFromBigInt(4095653410, 4),].expand((element) => element).toList();
  }
...
class ContentSettings {
  ...
  static ContentSettings fromReader(BinaryReader reader) {
    ...
    return new ContentSettings(sensitiveEnabled: sensitiveEnabled, sensitiveCanChange: sensitiveCanChange);
  }

  List<int> getBytes() {
    return [readBufferFromBigInt(1474462241, 4), [0, 0, 0, 0],].expand((element) => element).toList();
  }
