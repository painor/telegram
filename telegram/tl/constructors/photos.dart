...
List<dynamic> users = [];len = reader.readInt();
	...
	List<int> getBytes(){return [readBufferFromBigInt(2378853029,4),readBufferFromBigInt(0x15c4b51c,4),readBufferFromBigInt(this.photos.length,4,little:true,signed:true),this.photos.map((x)=>x.getBytes()),readBufferFromBigInt(0x15c4b51c,4),readBufferFromBigInt(this.users.length,4,little:true,signed:true),this.users.map((x)=>x.getBytes()),].expand((element) => element).toList();}
...
List<dynamic> users = [];len = reader.readInt();
	...
	List<int> getBytes(){return [readBufferFromBigInt(539045032,4),this.photo.getBytes(),readBufferFromBigInt(0x15c4b51c,4),readBufferFromBigInt(this.users.length,4,little:true,signed:true),this.users.map((x)=>x.getBytes()),].expand((element) => element).toList();}
